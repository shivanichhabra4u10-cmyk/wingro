            ...
            {coachList.map(coach => (
              <div key={coach._id || coach.id} className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 flex flex-col items-center">
                ...
                <div className="flex gap-2 mt-auto">
                  ...
                  <Link to={`/book/${coach._id || coach.id}`} className="px-4 py-2 rounded-full bg-indigo-700 text-white font-semibold shadow hover:bg-indigo-800 transition">Book Appointment</Link>
                </div>
              </div>
            ))}
          </div>
