...
exports.assessmentController = {
     ...
     */
    submitIndividualAssessment: (req, res) => __awaiter(void 0, void 0, void 0, function* () {
        try {
            ...
            const { firstName, lastName, email, jobTitle, company, yearsExperience, individualType, educationLevel, linkedinUrl, category, assessmentType } = req.body;
            ...
            const assessment = yield models_1.IndividualAssessment.create({
                firstName,
                lastName,
                email,
                jobTitle,
     ...
     */
    submitOrganizationAssessment: (req, res) => __awaiter(void 0, void 0, void 0, function* () {
        try {
            // Check for validation errors
            const errors = (0, express_validator_1.validationResult)(req);
            if (!errors.isEmpty()) {
            ...
            // Extract form data from request body
            const { companyName, contactName, contactEmail, contactPhone, companySize, industry, challengeArea, message, organizationType, yearsFounded, teamSize, linkedinUrl, category, assessmentType } = req.body;
            ...
            // Create new assessment entry in MongoDB
            const assessment = yield models_1.OrganizationAssessment.create({
                companyName,
                contactName,
                contactEmail,
                contactPhone,
     ...
     */
    updateIndividualAssessment: (req, res) => __awaiter(void 0, void 0, void 0, function* () {
        try {
            const { id } = req.params;
            ...
            const result = yield models_1.IndividualAssessment.updateOne({ _id: id }, {
                $set: {
            ...
            const updatedAssessment = yield models_1.IndividualAssessment.findById(id);
     ...
     */
    updateOrganizationAssessment: (req, res) => __awaiter(void 0, void 0, void 0, function* () {
        try {
            const { id } = req.params;
            const { responseData } = req.body;
            // Enhanced logging for debugging
            ...
            console.log('Using direct MongoDB update for reliability');
            // Use direct MongoDB update for maximum reliability
            const result = yield models_1.OrganizationAssessment.updateOne({ _id: id }, { $set: { responseData: responseData } }, { upsert: false });
            console.log('Update result:', result);
            if (result.matchedCount === 0) {
            ...
            // Verify the update worked by fetching it again
            const updatedAssessment = yield models_1.OrganizationAssessment.findById(id);
            if (updatedAssessment) {
