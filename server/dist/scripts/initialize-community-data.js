...
function initializeData() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const segmentsCount = yield communitySegment_model_1.CommunitySegmentModel.countDocuments();
            const postsCount = yield communityPost_model_1.CommunityPostModel.countDocuments();
            ...
            yield communitySegment_model_1.CommunitySegmentModel.create(sampleSegments);
            console.log(`Created ${sampleSegments.length} segments`);
            // Create posts
            console.log('Creating community posts...');
            const createdPosts = yield communityPost_model_1.CommunityPostModel.create(samplePosts);
            ...
            yield comment_model_1.CommentModel.create(sampleComments);
            console.log(`Created ${sampleComments.length} comments`);
            // Update comment counts on posts
            for (const post of createdPosts) {
                const commentCount = yield comment_model_1.CommentModel.countDocuments({ postId: post._id, isActive: true });
                if (commentCount > 0) {
                    yield communityPost_model_1.CommunityPostModel.findByIdAndUpdate(post._id, { comments: commentCount });
